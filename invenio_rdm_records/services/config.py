...
from invenio_records_resources.services.files.links import FileLink, FileLink2
...
class ThumbnailLinks:
    ...
    Adopts the interface of a Link but not one.
    """

    link_for_thumbnail = Link2(
        "iiif.image_api",
...
def get_file_links_list(name_of_blueprint):
    ...
    return {
        "self": Link2(f"{name_of_blueprint}.search", params=["pid_value"]),
        "archive": Link2(
...
def get_file_links_item(name_of_blueprint):
    ...
    # method's input. Therefore we don't pass a vars func for `pid_value`.
    return {
        "self": FileLink2(f"{name_of_blueprint}.read", params=["pid_value", "key"]),
        "content": FileLink2(
            ...
            vars=lambda file_drafcord, vars: vars.update(
                {
                    ...
                    "file_name": vars["key"],  # Because of FileLink2
                }
            ),
        ),
        "iiif_base": Link2(
...
class RDMRecordServiceConfig(RecordServiceConfig, ConfiguratorMixin):
    ...
    links_item = {
        ...
        "self": ConditionalLink(
            cond=is_record,
            if_=RecordLink2("records.read"),
            else_=RecordLink2("records.read_draft"),
        ),
        "self_html": ConditionalLink(
            cond=is_record,
            if_=RecordLink2("invenio_app_rdm_records.record_detail"),
            else_=RecordLink2("invenio_app_rdm_records.deposit_edit"),
        ),
        "preview_html": RecordLink2(
        ...
        # IIIF
        "self_iiif_manifest": Link2(
            "iiif.manifest", params=["uuid"], vars=vars_self_iiif
        ),
        "self_iiif_sequence": Link2(
            "iiif.sequence", params=["uuid"], vars=vars_self_iiif
        ),
        # Files
        "files": ConditionalLink(
            cond=is_record,
            if_=RecordLink2("record_files.search"),
            else_=RecordLink2("draft_files.search"),
        ),
        "media_files": ConditionalLink(
            cond=is_record,
            if_=RecordLink2("record_media_files.search"),
            else_=RecordLink2("draft_media_files.search"),
        ),
        ...
        # Versioning
        "latest": RecordLink2("records.read_latest", when=is_record),
        "latest_html": RecordLink2(
            "invenio_app_rdm_records.record_latest",
            when=is_record,
        ),
        "versions": RecordLink2("records.search_versions"),
        # Corresponding Draft/Record
        "draft": RecordLink2("records.read_draft", when=is_record),
        "record": RecordLink2("records.read", when=is_draft),
        # TODO: record_html temporarily needed for DOI registration, until
        # problems with self_doi has been fixed
        "record_html": RecordLink2(
            "invenio_app_rdm_records.record_detail", when=is_draft
        ),
        # Actions
        "publish": RecordLink2("records.publish", when=is_draft),
        "review": RecordLink2("records.review_read", when=is_draft),
        "submit-review": RecordLink2(
            "records.review_submit",
            when=is_draft_and_has_review,
        ),
        # Access
        "access_links": RecordLink2("record_links.search"),
        "access_grants": RecordLink2("record_grants.search"),
        "access_users": RecordLink2("record_user_access.search"),
        "access_groups": RecordLink2(
            "record_group_access.search",
            when=_groups_enabled,
        ),
        "access_request": RecordLink2("records.create_access_request"),
        "access": RecordLink2("records.update_access_settings"),
        # Communities
        "communities": RecordLink2("record_communities.search"),
        "communities-suggestions": RecordLink2("record_communities.get_suggestions"),
        # Requests
...
class RDMCommunityRecordsConfig(BaseRecordServiceConfig, ConfiguratorMixin):
    ...
    links_search_community_records = pagination_links2(
        "community_records.search", params=["pid_value"]
    )
